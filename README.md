...
The required fields are: a unique identifier (`UID`), an identifier for the analytical session (`Session`), a sample identifier (`Sample`), and the working-gas delta values `d45`, `d46`, `d47`.

Independently known oxygen-17 anomalies may be provided as `D17O` (in ‰ relative to VSMOW, λ = 0.528), and are assumed to be zero otherwise. Working-gas deltas `d48` and `d49` may also be provided, and are also set to 0&sbsp;‰otherwise.
...
#### 3.2.2 Option 2: based on the known composition of a sample:

```python
# The 4 code lines below are the default settings. It is thus not
# necessary to include them unless you wish to use different values.

foo.SAMPLE_CONSTRAINING_WG_COMPOSITION = 'ETH-3'
foo.d13C_VPDB_OF_SAMPLE_CONSTRAINING_WG_COMPOSITION = 1.71	# (Bernasconi et al., 2018)
foo.d18O_VPDB_OF_SAMPLE_CONSTRAINING_WG_COMPOSITION = -1.78	# (Bernasconi et al., 2018)
...
# e["d48"] = 24.56708
# e["d49"] = 0.79486
# e["d13Cwg_VPDB"] = -3.7555729459832765
# e["d18Owg_VSMOW"] = 25.1145492463934
# e["D17O"] = 0.0
# e["d13C_VPDB"] = 1.9948594073404546
# e["d18O_VSMOW"] = 37.03357105550355
# e["D47raw"] = -0.5746856128030498
# e["D48raw"] = 1.1496833191546596
# e["D49raw"] = -27.690248970251407
```

### 3.4 Oxygen-17 correction parameters
...
### 3.6 Standardization (`integrated_fit`)

The default standardization step computes the best-fit standardization parameters (a,b,c) for each session, along with the best-fit Δ<sub>47</sub> values of unknown samples,
using an integrated regression model taking into account the relative mapping of all samples (anchors and unknowns) in (δ<sub>47</sub>, Δ<sub>47</sub>) space.
...
External reproducibility of δ13C_VPDB           13.8 ppm
External reproducibility of δ18O_VSMOW          41.9 ppm
External reproducibility of Δ47 (anchors)       10.7 ppm
External reproducibility of Δ47 (unknowns)       3.4 ppm
External reproducibility of Δ47 (all)            8.6 ppm
Degrees of freedom (Student's 95% t-factor)    12 (2.18)
-------------------------------------------  -----------
...
+ `r_d13C_VPDB`, `r_d18O_VSMOW`, `r_D47`: external reproducibilities for `d13C_VPDB`, `d18O_VSMOW`, `D47` in this session

### 3.6.2 `D47data.samples`, `D47data.anchors`, and `D47data.unknowns`
...
The overall analytical reproducibilities are now saved to `foo.repro`:

```python
for k in foo.repro:
    print(f"{k:>12}: {foo.repro[k]}")
...
+ `r_d13C_VPDB`: External reproducibility of δ<sup>13</sup>C for all samples
+ `r_d18O_VSMOW`: External reproducibility of δ<sup>18</sup>O for all samples (CO<sub>2</sub> values)
+ `r_D47a`: External reproducibility of Δ<sub>47</sub> for anchor samples only
+ `r_D47u`: External reproducibility of Δ<sub>47</sub> for unknown samples only
+ `r_D47`: External reproducibility of Δ<sub>47</sub> for all samples.

### 3.6.4 `D47data.result`
